...
static void fade_leds(struct color *framebuffer, struct game *game, struct payload *p)
{
    ...
    for (i = 0; i < game->player_count; i++)
    {
        ...
        p->zero_counter += ll_led_fade_leds_for_player(framebuffer, i, LL_LED_FADE_DIR_OUT, 15, 3);
    }
}
...
static void player_switch(struct color *framebuffer, struct game *game, struct payload *p, uint32_t type)
{
    ...
    for(uint32_t i = LL_LED_NUM_ALL_BARS_CIRCLES; i < LL_LED_NUM_LEDS; i++)
    {
        ll_led_set_alpha(framebuffer, alpha, i);
    }
}
...
struct animation *anim_player_lost_alone_init(struct color *framebuffer)
{
